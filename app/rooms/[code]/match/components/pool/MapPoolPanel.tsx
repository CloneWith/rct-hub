 ...
 *
 * Interaction (M2): `selectableSlotIDs` marks slots the current actor may
 * target (ban candidates, or pickable slots), `selectedSlotID` highlights the
 * currently selected pick slot, and `onSelectSlot` reports clicks. The panel
 * does not judge legality itself — that comes from `analysis` (D5).
 */
...
function SlotCard({
  ...
  const picked = slot.state === "SELECTED";

  return (
    <div
      role={selectable ? "button" : undefined}
      aria-label={selectable ? `选择槽位 ${slot.id}` : undefined}
      onClick={selectable ? () => onSelect?.(slot.id) : undefined}
      className={`flex items-center gap-2 rounded-lg border p-2 transition-shadow ${
        selected
          ? "border-success/70 bg-background/70 ring-1 ring-success/60"
          : banned
            ? "border-transparent bg-black/30 opacity-45"
            : picked
              ? "border-border bg-background/60"
              : selectable
                ? "cursor-pointer border-border bg-background/40 hover:ring-1 hover:ring-success/50"
    ...
    >
      ...
      {picked && <span className="shrink-0 text-xs font-semibold text-success">已上场</span>}
      {selected && <span className="shrink-0 text-xs font-semibold text-success">已选 · 点击棋盘落子</span>}
    </div>
  );
}
